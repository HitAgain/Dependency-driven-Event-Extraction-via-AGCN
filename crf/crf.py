...
class CRF(Layer):
    ...
    def loss_function(self):
        if self.learn_mode == 'join':
            def loss(y_true, y_pred):
                ...
                X = self._inbound_nodes[0].input_tensors
                mask = self._inbound_nodes[0].input_tensors._keras_mask
    ...
    def add_boundary_energy(self, energy, mask, start, end):  # energy->dense output [batch, seq, dim]
        start = K.expand_dims(K.expand_dims(start, 0), 0)  # [0, 0, dim]
        end = K.expand_dims(K.expand_dims(end, 0), 0)  # [0, 0, dim]
        if mask is None:
            ...
            # end_mask = K.cast(K.greater(self.shift_left(mask), mask), K.floatx())  # 可能存在bug
    ...
    def step(self, input_energy_t, states, return_logZ=True):
        ...
        # input_energy_t是LSTM编码得到的向量，就是H
        ...
        if return_logZ:
            ...
            new_target_val = tf.math.reduce_logsumexp(-energy, 1, keepdims=False)  # shapes: (B, F)
